...
/**
 * Pinterest connector backed by Buffer.
 *
 * Buffer support level for Pinterest:
 *  - publishPost:    full (text + image required for Pin)
 *  - schedulePost:   full
 ...
 *
 * Note: Pinterest Pins require an image. If no media is provided, the post
 * will be created as a text-only Pin (Buffer may reject it — callers should
 * always provide at least one image asset for Pinterest).
 */
export class BufferPinterestConnector implements Connector {
  ...
  async publishPost(post: PostPayload): Promise<PublishResult> {
    const profileId = await resolveProfileId("pinterest");
