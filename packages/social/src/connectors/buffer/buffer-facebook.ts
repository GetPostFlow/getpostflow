...
/**
 * Facebook connector backed by Buffer.
 *
 * Buffer support level for Facebook:
 *  - publishPost:    full (text + first image via media[photo])
 *  - schedulePost:   full
 *  - fetchAnalytics: partial (reach, clicks via update statistics)
 ...
 *
 * Profile resolution: looks for the first Buffer profile with service="facebook".
 */
export class BufferFacebookConnector implements Connector {
  ...
  async publishPost(post: PostPayload): Promise<PublishResult> {
    const profileId = await resolveProfileId("facebook");
