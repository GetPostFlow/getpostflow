...
/**
 * Instagram connector backed by Buffer.
 *
 * Buffer support level for Instagram:
 *  - publishPost:    full (text + image; video requires Buffer business plan)
 *  - schedulePost:   full
 ...
 */
export class BufferInstagramConnector implements Connector {
  ...
  async publishPost(post: PostPayload): Promise<PublishResult> {
    const profileId = await resolveProfileId("instagram");
